...
3. **Class Comparison**: Khi tìm được class SinhVien tương ứng `MappedNode`:
   - So sánh Constructor: Nếu mẫu không có thì auto qua, nếu có thì so trùng kiểu params (áp dụng logic OR nếu có nhiều phương án `|`, KHÔNG cần đúng thứ tự) và Scope.
   - So sánh Attribute: Match bằng **Type** (hỗ trợ OR-logic `|`, sau TypeMap) rồi **Name** (fuzzy `|`), sau đó kiểm tra Scope và Kind -> `WrongDetail/CorrectDetail`.
   - So sánh Method Set: Match bằng **ReturnType + ParamCount (+-1 nếu cả hai >=2)** rồi **Name** (fuzzy `|`), sau đó kiểm tra Scope, Kind, **Params chính xác (mỗi param đều hỗ trợ OR-logic `|`)** -> `WrongDetail/CorrectDetail`. Getter/Setter chỉ đếm số lượng.
